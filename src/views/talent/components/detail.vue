...
<script>
import {recruit_id,dynamic_id} from "@/api/wz/wzTalent"
export default {
  props: {
    item: {
      type: Object,
  ...
  methods: {
    getDetail(){
      if(this.item.link=='recruit'){
        recruit_id(this.$route.query.id).then((response) => {
          if (response.code === 200) {
            this.detail=response.data;
          }
        });
      }else if(this.item.link=='dynamic'){
        dynamic_id(this.$route.query.id).then((response) => {
          if (response.code === 200) {
            this.detail=response.data;
          }
        });
