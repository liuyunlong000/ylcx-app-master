import request from '@/utils/request';

/**
 * 添加登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function save_id(data:any) {
    return request({
        url: '/sysCommonLog/save',
        method: 'post',
        data,
    });
}

/**
 * 编辑登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function update_id(data:any) {
    return request({
        url: '/sysCommonLog/update',
        method: 'put',
        data,
    });
}

/**
 * 按id删除登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function delete_id(data:any) {
    return request({
        url: '/sysCommonLog/delete',
        method: 'delete',
        params: data,
    });
}

/**
 * 按id集合批量删除登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function batDelete(data:any) {
    return request({
        url: '/sysCommonLog/bat_delete',
        method: 'delete',
        data,
    });
}

/**
 * 按id查询登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function find_id(data:any) {
    return request({
        url: '/sysCommonLog/find_id',
        method: 'get',
        params: data,
    });
}

/**
 * 获取登录日志列表
 * @param data
 * @returns {AxiosPromise}
 */
export function find_list(data:any) {
    return request({
        url: '/sysCommonLog/find_list',
        method: 'post',
        data,
    });
}

/**
 * 获取登录日志列表（分页）
 * @param data
 * @returns {AxiosPromise}
 */
export function find_list_by_page(data:any) {
    return request({
        url: '/sysCommonLog/find_list_by_page',
        method: 'post',
        data,
    });
}

/**
 * 按id启用或禁用登录日志
 * @param data
 * @returns {AxiosPromise}
 */
export function disable_id(data:any) {
    return request({
        url: '/sysCommonLog/disable',
        method: 'get',
        params: data
    });
}
